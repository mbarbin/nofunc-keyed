...
(* NOTE: If this file is hashtbl.mli, do not edit it directly! Instead, edit
   templates/hashtbl.template.mli and run tools/sync_stdlib_docs *)
...
(** [Hashtbl.add tbl key data] adds a binding of [key] to [data] in table [tbl].
    ...
    That is, after performing {!remove}[ tbl key], the previous binding for
...
(** [Hashtbl.filter_map_inplace f tbl] applies [f] to all bindings in table
    [tbl] and update each binding depending on the result of [f]. If [f] returns
    [None], the binding is discarded. If it returns [Some new_val], the binding
    is update to associate the key to [new_val].
...
(** [Hashtbl.hash x] associates a nonnegative integer to any value of any type.
    It is guaranteed that if [x = y] or [Stdlib.compare x y = 0], then
...
(** [Hashtbl.hash_param meaningful total x] computes a hash value for [x], with
    ...
    {!hash} and {!seeded_hash} take [meaningful = 10] and [total = 100]. *)

val seeded_hash_param : int -> int -> int -> 'a -> int
(** A variant of {!hash_param} that is further parameterized by an integer seed.
