...
val add : compare:'key compare -> 'key -> 'a -> ('key, 'a) t -> ('key, 'a) t
...
val add_to_list :
  compare:'key compare -> 'key -> 'a -> ('key, 'a list) t -> ('key, 'a list) t
...
val remove : compare:'key compare -> 'key -> ('key, 'a) t -> ('key, 'a) t
...
val find : compare:'key compare -> 'key -> ('key, 'a) t -> 'a
(** [find x m] returns the current value of [x] in [m], or raises [Not_found] if
    no binding for [x] exists. *)

val find_opt : compare:'key compare -> 'key -> ('key, 'a) t -> 'a option
...
(** [find_first f m], where [f] is a monotonically increasing function, returns
    ...
    first binding [k, v] of [m] where [Ord.compare k x >= 0] (intuitively:
...
val is_singleton : ('key, 'a) t -> bool
(** Test whether a map has exactly one element or not.

    @since 5.5 *)

val mem : compare:'key compare -> 'key -> ('key, 'a) t -> bool
...
val to_seq_from :
  compare:'key compare -> 'key -> ('key, 'a) t -> ('key * 'a) Seq.t
(** [to_seq_from k m] iterates on a subset of the bindings of [m], in ascending
    order of keys, from key [k] or above.
    @since 4.07 *)

val add_seq :
  compare:'key compare -> ('key * 'a) Seq.t -> ('key, 'a) t -> ('key, 'a) t
(** Add the given bindings to the map, in order.
    @since 4.07 *)

val of_seq : compare:'key compare -> ('key * 'a) Seq.t -> ('key, 'a) t
