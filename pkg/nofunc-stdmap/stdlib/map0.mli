...
(* NOTE: If this file is map.mli, do not edit it directly! Instead, edit
   templates/map.template.mli and run tools/sync_stdlib_docs *)
...
module type OrderedType = sig
  ...
  (** [add key data m] returns a map containing the same bindings as [m], plus a
      ...
      that is physically equal to [data], [m] is returned unchanged (the result
      of the function is then physically equal to [m]). Otherwise, the previous
  ...
  (** [update key f m] returns a map containing the same bindings as [m], except
      ...
      If [y] is [None], the binding is removed if it exists; otherwise, if [y]
      is [Some z] then [key] is associated to [z] in the resulting map. If [key]
      was already bound in [m] to a value that is physically equal to [z], [m]
  ...
  (** [merge f m1 m2] computes a map whose keys are a subset of the keys of [m1]
      ...
      operation, we have
      [find_opt x (merge f m1 m2) = f x (find_opt x m1) (find_opt x m2)] for any
      key [x], provided that [f x None None = None].
      @since 3.12 *)
  ...
  (** Same as {!min_binding}, but returns the binding with the largest key in
      the given map.
      @since 3.12 *)

  val max_binding_opt : 'a t -> (key * 'a) option
  (** Same as {!min_binding_opt}, but returns the binding with the largest key
      in the given map.
      @since 4.05 *)
  ...
  val find : key -> 'a t -> 'a
  (** [find x m] returns the current value of [x] in [m], or raises [Not_found]
  ...
  (** [partition f m] returns a pair of maps [(m1, m2)], where [m1] contains all
      the bindings of [m] that satisfy the predicate [f], and [m2] is the map
  ...
  (** [of_list bs] adds the bindings of [bs] to the empty map, in list order (if
      a key is bound twice in [bs] the last one takes over).
      @since 5.1 *)
