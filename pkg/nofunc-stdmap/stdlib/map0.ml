...
(* Beware: those two functions assume that the added k is *strictly* smaller (or
   bigger) than all the present keys in the tree; it does not test for equality
   with the current min (or max) key.
...
let rec filter_map f = function
  ...
  | Node { l; v; d; r } ->
      ...
      begin match fvd with Some d' -> join l' v d' r' | None -> concat l' r'
      end
