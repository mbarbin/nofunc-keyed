...
(* NOTE: If this file is set.mli, do not edit it directly! Instead, edit
   templates/set.template.mli and run tools/sync_stdlib_docs *)
...
(** Sets over ordered types.
    ...
    trees, and is therefore reasonably efficient: insertion and membership take
    time logarithmic in the size of the set, for instance.
    ...
    This creates a new module [PairsSet], with a new type [PairsSet.t] of sets
    of [int * int]. *)
...
module type OrderedType = sig
  ...
  val add : elt -> t -> t
  (** [add x s] returns a set containing all elements of [s], plus [x]. If [x]
      was already in [s], [s] is returned unchanged (the result of the function
  ...
  (** [remove x s] returns a set containing all elements of [s], except [x]. If
      [x] was not in [s], [s] is returned unchanged (the result of the function
      is then physically equal to [s]).
      @before 4.03 Physical equality was not ensured. *)
  ...
  (** [find_first f s], where [f] is a monotonically increasing function,
      returns the lowest element [e] of [s] such that [f e], or raises
  ...
  (** [find_last f s], where [f] is a monotonically decreasing function, returns
      the highest element [e] of [s] such that [f e], or raises [Not_found] if
  ...
  (** [map f s] is the set whose elements are [f a0],[f a1]... [f aN], where
      ...
      If no element of [s] is changed by [f], [s] is returned unchanged. (If
      each output of [f] is physically equal to its input, the returned set is
  ...
  (** [split x s] returns a triple [(l, present, r)], where [l] is the set of
      ...
      elements of [s] that are strictly greater than [x]; [present] is [false]
      if [s] contains no element equal to [x], or [true] if [s] contains an
      element equal to [x]. *)
